...
class DistTrainer(object):
    ...
    def build_optimizer(self,
        ...
        if optimizer == "DSGD":
            ...
            return AdmmSGD(self.nodename,
                        ...
                        device=self.device,
                        **args)
        elif optimizer == "PdmmSGD":
            from optimizer.pdmm_sgd import PdmmSGD
            return PdmmSGD(self.nodename,
                        round_cnt,
                        self.edges,
                        self.hosts,
                        self.model,
                        device=self.device,
                        **args)
        elif optimizer in ["AdmmISVR"]:
            from optimizer.admm_isvr import AdmmISVR
            return AdmmISVR(self.nodename,
                       round_cnt,
                       self.edges,
                       self.hosts,
                       self.model,
                       device=self.device,
                       **args)
        elif optimizer in ["PdmmISVR"]:
            from optimizer.pdmm_isvr import PdmmISVR
            return PdmmISVR(self.nodename,
                       round_cnt,
                       self.edges,
                       self.hosts,
                       self.model,
    ...
    def load_data(self, indices: typing.List[int] = None, train_data_length: int = None):
    ...
    def load_datas(self,
        ...
        if self.data_split_mode == "split":
            ...
            return self.load_data(indices=indices, train_data_length=train_data_length)
    ...
    def split_load_datas(self,
        ...
        for node_idx, data_indices in enumerate(node_data_indices):
            ...
            logging.info("train data node%d: %s" % (node_idx, ",".join(["%d:%d" % (i, n) for i, n in zip(classes, counts)])))
        node_indices = node_data_indices[self.nodeindex]
        datasets = self.load_data(indices=node_indices, train_data_length=train_data_length)

        logging.info("split_load_datas(END)")
        return datasets
    ...
    def split_class_data_same_load_datas(self,
        ...
        node_indices = node_data_indices[self.nodeindex]
        datasets = self.load_data(indices=node_indices,
    ...
    def train(self, epochs: int, batch_size: int,
        ...
        logging.info("build_criterion(1)")
        ...
        logging.info("build_criterion(2)")

        self.__optim__ = self.build_optimizer(optimizer=optimizer,
                                              args=optim_args)
        logging.info("build_optimizer()")
